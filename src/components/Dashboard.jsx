...
const Dashboard = () => {
	...
	useEffect(() => {
		...
		const ws = new WebSocket(`ws://localhost:8000/ws/socket-server/${sessionid}/`);
	...
	const getStatusText = (studentId) => {
		...
		const status = statuses.find(s => s.id === parseInt(attendanceRecord.statusid));
	...
	const renderStudentTable = (title, filterStatus) => {
		const filteredStudents = students.filter(student => {
			...
			if (filterStatus === 'present') {
				...
				return statusId === statuses.find(s => s.acronym === "A")?.id ||
